...
export function OrbitusVM() {
  ...
  useEffect(() => {
    const loadVms = async () => {
        setIsLoading(true);
        const storedVms = await get<VM[]>('orbitus-vms');
        if (storedVms) {
            setVms(storedVms);
        } else {
            ...
            setVms([defaultVm]);
            if (!isGuest) await set('orbitus-vms', [defaultVm]);
        }
        setIsLoading(false);
    };
    loadVms();
  ...
  const handleCreateVM = async () => {
      ...
      if (!isGuest) {
          await set('orbitus-vms', updatedVms);
      }
  ...
  const handleDeleteVM = async (id: string, e: React.MouseEvent) => {
      ...
      setVms(updatedVms);
      if (!isGuest) await set('orbitus-vms', updatedVms);
  };
  ...
  const getCustomVmSrcDoc = (vm: VM) => `
    ...
    <body>
        <div id="status">ORBITUS LOCAL HYPERVISOR v3.0 // ARCH: ${vm.arch} // MEM: ${vm.memory}</div>
                ...
                {isBooting ? (
                  <div className="absolute inset-0 z-10 bg-black flex flex-col items-center justify-center gap-4">
                    <div className="font-mono text-xs text-green-500 space-y-1">
                      ...
                      <p>[    0.004512] Orbitus Hypervisor detected</p>
